...
def test_wrapper(loader, model, ad_net_list, test_10crop=True, gpu=True, ad_local_num=7, conv_feature_dim=2048):
        ...
        for i in range(len(loader['test0'])):#300
            ...
            for j in range(1):
                ...
                for k in range(len(ad_net_list)):
                    ...
                    attention_region.append(ad_entropy_local)  # cross-entropy transfer
                ...
                attention_pool_local = attention_pool.view(len(attention_pool), -1)  ## 7*7 for resnet; 6*6 for alexnet
    ...
    C_M = confusion_matrix(label_source_np, predict_source_np) #confusion_matrix
    Kappa = kappa(C_M)  #kappa number
    ...
    OA = sum(per_class)/len(per_class)    #OA
...
##### set loss
    ...
    loss_params = config["loss"] #1.0 and 0.1
...
##### add additional network for some methods
    ...
    for i in range(ad_local_num * ad_local_num):  ## different datasets have different types of adversarial network with various depth
...
##### train process
    ...
    best_acc = 0.0
    iter_num = 0.0
    for i in range(config["num_iterations"]):#4000
  ######test
        ...
        if test_model ==True:
            if i % config["test_interval"] == 0 and i > 0:
                ...
                t_model = nn.Sequential(base_network)
                checkpt = torch.load("/home/x/D/newb/data/MRDAN/test_model/U2R/best_model.pth.tar")
                t_model.load_state_dict(checkpt, strict=False)#
  ...
  ##### train one iter
        ...
        ##### concatenate inputs_source and inputs_target
...
###
    ...
    parser.add_argument('--Output_str', type=str, default='MRDAN_A2U_test', help="entropy loss trade off")
    ...
    parser.add_argument('--datadir', type=str, default='/home/x/D/newb/data/3TADA/3TADA-code/data/', help="the root datadir of the dataset")
    ...
    config["loss"] = {"entropy_trade_off": args.entropy_trade_off}
